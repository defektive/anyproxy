  ...
  getServer() {
    ...
    app.get('/fetchBody', (req, res) => {
      ...
      if (query && query.id) {
        recorder.getDecodedBody(query.id, (err, result) => {
          // 返回下载信息
          if (err && !result) {
            // we may not have the body yet. eg: intercepted request
            result = {content: ''}
          }
          ...
          if (result.statusCode === 200 && result.mime) {
            if (result.type === 'json' ||
    ...
    app.get('/api/getInitData', (req, res) => {
      ...
      res.json({
        ...
        interactiveIntercept: global._dirtyHack.interactiveIntercept
      });
    });
    ...
    app.get('/api/status', (req, res) => {
      res.json( global._dirtyHack );
    });

    app.get('/api/intercept', (req, res) => {
      res.json({intercepting: global._dirtyHack.interactiveIntercept });
    });

    app.post('/api/intercept', (req, res) => {
      global._dirtyHack.interactiveIntercept = !!req.body.intercepting;
      if (!global._dirtyHack.interactiveIntercept) {
        let req;
        let keys = Object.keys(global._dirtyHack.interceptedRequests);
        while(keys.length) {
          req = global._dirtyHack.interceptedRequests[keys.shift()];
          req.resolve({});
        }
      }
      res.json({intercepting: global._dirtyHack.interactiveIntercept });
    ...
    app.post('/api/request/:requestId', (req, res) => {
      const reqId = req.params.requestId;
      let ir = global._dirtyHack.interceptedRequests[reqId];
      if (ir) {
        let requestOptions = ir.requestDetail.requestOptions;
        ir.resolve(req.body.modifiedRequest || {path:'/anything/pizza'});
        // setTimeout(() => {
          global._dirtyHack.interceptedRequests[reqId] = null;
          delete global._dirtyHack.interceptedRequests[reqId];
        // }, 1000);
        res.json(req.body.modifiedRequest || {});
      } else {
        recorder.getSingleRecord(reqId, (err, doc) => {
          if (err) {
            res.end(err.toString());
          } else {
